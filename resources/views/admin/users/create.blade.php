<x-splade-modal>
    ...
    <x-splade-form :action="route('admin.users.store')" class="p-4 bg-white rounded-md space-y-2">
        ...
        <x-splade-select name="roles[]" :options="$roles" multiple relation choices label="User Roles" />
        <x-splade-select name="permissions[]" :options="$permissions" multiple relation choices label="User Permissions" />
        <x-splade-submit />
    </x-splade-form>
</x-splade-modal>
